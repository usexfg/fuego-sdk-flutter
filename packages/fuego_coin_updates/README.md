...
This package fetches the unified coins configuration JSON from the `usexfg/coins` repository (`utils/coins_config_unfiltered.json` by default), converts entries into `Asset` models (from `fuego_defi_types`), persists them to Hive, and tracks the source commit so you can decide when to refresh.

## Features

- Fetch latest commit from the `usexfg/coins` repo
...
final config = AssetRuntimeUpdateConfig(
  ...
  coinsRepoApiUrl: 'https://api.github.com/repos/usexfg/coins',
  coinsRepoContentUrl: 'https://raw.githubusercontent.com/usexfg/coins',
