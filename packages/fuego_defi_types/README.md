...
Lightweight, shared domain types used across the Fuego DeFi SDK and Framework. These types are UI- and storage-agnostic by design.

## Install

```sh
dart pub add fuego_defi_types
```
...
These types are consumed by higher-level managers in `fuego_defi_sdk`.
