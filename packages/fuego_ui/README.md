...
Reusable Flutter widgets for DeFi apps built on the Fuego DeFi SDK and Framework. Focused, production-ready components that pair naturally with the SDK’s managers.
...
Widgets are framework-agnostic and can be used directly. When used with the SDK, adapter widgets are available from `fuego_defi_sdk` to bind to SDK streams, e.g.:

```dart
// From fuego_defi_sdk: live balance text bound to BalanceManager
