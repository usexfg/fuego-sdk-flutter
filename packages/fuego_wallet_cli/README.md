...
dart run packages/komodo_wallet_cli/bin/komodo_wallet_cli.dart get \
  --all \
  --artifact_output_package=fuego_defi_framework \
...
dart run packages/komodo_wallet_cli/bin/update_api_config.dart \
  --branch dev \
  --source mirror \
  --config packages/fuego_defi_framework/app_build/build_config.json \
  --output-dir packages/fuego_defi_framework/app_build/temp_downloads \
  --verbose
```
...
```sh
komodo_wallet_cli update_api_config --branch dev --source mirror --config packages/fuego_defi_framework/app_build/build_config.json
```

Options:

- `-b, --branch <name>` – Branch to fetch commit from (default: master)
- `--repo <owner/repo>` – Repository (default: usexfg/komodo-defi-framework)
...
- `--mirror-url <url>` – Mirror base URL (default: https://devbuilds.usexfg.org)
